        ...
        static void Main(string[] args)
        {
            ...
            List<Student> studentList = new List<Student>()
          {
            ...
            Console.WriteLine("1. 列出所有課程的名稱");
            {
                //作答區
                //...
                var courseNames = courseList.Select(course => course.Name).ToList();
                courseNames.ForEach(courseName => Console.WriteLine(courseName));
            }
            ...
            {
                //作答區
                var courseIds = courseList.Where(course => course.Classroom == "L107")
                             .Select(course => course.CourseId)
                             .ToList();
                courseIds.ForEach(courseId => Console.WriteLine(courseId));
            ...
            {
                //作答區
                var courseIDs = courseList.Where(course => course.Classroom == "L107" && course.Credit == 3)
                              .Select(course => course.CourseId)
                              .ToList();
                courseIDs.ForEach(courseId => Console.WriteLine(courseId));
            }
            ...
            {
                //作答區
                var teachers = courseList.Select(course => course.Teacher)
                             .Distinct()
                             .ToList();
                teachers.ForEach(teacher => Console.WriteLine(teacher));
            }
            ...
            {
                //作答區
                var teachers = courseList.Where(course => course.Classroom == "L202")
            ...
            {
                //作答區
                var femal = studentList.Where(student => student.Gender == GenderOption.Female)
                                    .Select(student => student.Name)
                                    .ToList();
                femal.ForEach(name => Console.WriteLine(name));
            }
            ...
            {
                //作答區
                var InGit = studentList.Where(student => student.CourseList.Any(courseId =>
                                  courseList.Any(course => course.CourseId == courseId && course.Name == "Git")))
                                  .Select(student => student.Name)
                                  .ToList();
                InGit.ForEach(name => Console.WriteLine(name));
            }
            ...
            Console.WriteLine("8. 列出每個學員上的每一堂課");
            {
                ...
                foreach (var student in studentList)
                {
                    ...
                    var courses = student.CourseList.Select(courseId => courseList.First(course => course.CourseId == courseId).Name).ToList();
                    courses.ForEach(courseName => Console.WriteLine($"  {courseName}"));
                }
            }
            ...
            {
                //作答區
                var LeastCourse = studentList.OrderBy(student => student.CourseList.Count).FirstOrDefault();
                if (LeastCourse != null)
                {
                    Console.WriteLine($"{LeastCourse.Name}");
                }
            }
            ...
            {
                //作答區
                var LessThanTen = studentList.Where(student =>
                           student.CourseList.Sum(courseId => courseList.First(course => course.CourseId == courseId).Credit) < 10)
                           .ToList();
                LessThanTen.ForEach(student => Console.WriteLine(student.Name));
            }
